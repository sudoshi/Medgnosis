...
4. **Dashboard aggregates** (`phm_star.fact_patient_composite` + materialized views) — NOT rebuilt. The synthetic patients are visible in the Patients list (admin) + Patient Detail (direct `phm_edw` reads), but dashboard risk/care-gap aggregates and Solr name-search use the batch-built star schema / Solr index. Refreshing those is a full star-schema ETL over ~1M patients (heavy, observation-table-I/O-intensive) — not warranted for 7 test patients. Deferred by design.
