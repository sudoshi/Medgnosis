...
- [ ] **D4. Problem-list curation layer** — curation over existing `phm_edw.problem_list` (12,067 rows) with provenance (clinician / auto-load / recommendation-accepted), bulk load utility (add / resolve / add-resolve-move-to-history) with per-chart audit. (C12)
