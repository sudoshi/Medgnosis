...
### Foundations
- [ ] **D1. Rules-as-data engine** — `app.clinical_rule` EAV table (entity/attribute/value, effective/expiration dates, version, author), evaluation service `rulesEngine.evaluate(entity, attribute, asOfDate?)`, REST endpoint to *show the criteria behind any computation* ("transparency → trust"), migrate the hardcoded alert rules into it. (C2)
- [ ] **D2. Diagnosis ontology** — `app.dx_ontology` (code → disease_process → organ_system → generate_plan), staged taxonomies for CKD (GFR-staged), heart failure (function × etiology, 18 dx), obesity (BMI-banded) in SNOMED CT + ICD-10-CM. (C12)
- [ ] **D3. Appointment data model** — `phm_edw.appointment` + `phm_star.fact_appointment`, synthetic appointment generator for the demo population, no-show tracking fields. (prereq for C7)
- [ ] **D4. Problem-list curation layer** — `app.problem_list_entry` over `phm_edw.condition` with provenance (clinician / auto-load / recommendation-accepted), bulk load utility (add / resolve / add-resolve-move-to-history) with per-chart audit. (C12)

### Population identification
- [ ] **D5. Two-pass population finder** — Pass 1: re-map existing generic diagnoses to staged taxonomy; Pass 2: compute evidence (eGFR→CKD stage, BMI from cleaned height/weight, EF→HF class) and propose problem-list additions. Worklist UI for clinician review before commit. (C12)
- [ ] **D6. Recommendation cards** — "evidence suggests X, not on problem list" with *Does not have X* as first-class dismissal + 12-month snooze, served via CDS Hooks card + in-app. (C11/C12)
...
### Closed-loop safety
- [ ] **D10. Close the Loop engine** — `app.result_loop` (result → guideline obligation → window → status), guideline matrices stored in the rules engine (start: cervical cytology per **current ASCCP 2019 risk-based guidelines**, designed as a content swap), closure-evidence search across orders/procedures/notes, CTL worklist with 4 terminal dispositions (appropriate care / clinical resolution / documented refusal / exhausted outreach). (C8)
- [ ] **D11. Population risk models** — Gail/BCRA (7-factor, 5-yr invasive breast cancer risk) over eligible women; surface chemoprevention gap. Pluggable model registry so CHA₂DS₂-VASc etc. follow the same pattern. (C15)
