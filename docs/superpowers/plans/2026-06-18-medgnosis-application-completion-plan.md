...
- [ ] Add admin UI for ingestion.
  ...
  - [ ] Expand patient/resource sync visibility with deeper conflict drilldowns and alerting.
...
- [ ] Add alerts, dead-letter runbook depth, deeper patient/resource drilldowns, and remaining EDW/local-matching breadth for tenant-specific patient-detail needs.
...
- EHR integration risk: SMART launch can appear complete while still lacking vendor sandbox evidence, remaining FHIR-read/token audit surfaces, durable patient/resource alerting/drilldowns, access-policy attribution, and tenant-specific EDW/local-matching breadth.
- Data risk: Bulk Data kickoff/poll/import/scheduling can create a false sense of production readiness until vendor sandbox evidence, deeper replay/dead-letter runbooks, tombstone behavior, stale-data alerting, and deeper patient/resource drilldowns are proven.
