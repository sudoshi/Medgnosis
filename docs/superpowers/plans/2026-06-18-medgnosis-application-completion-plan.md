...
- Auth/session audit coverage now captures PHI-safe known-user security events for local login failures, MFA failures, refresh-token lifecycle and replay handling, rejected password changes, session revoke misses, public registration creation, and preference updates while documenting no-audit rationale for anti-enumeration and untrusted-token branches. OIDC callback success audit details no longer persist raw email claims.
