...
- [ ] Add admin UI for ingestion.
  ...
  - [ ] Expand beyond tenant ingest/readiness/worker rollups with patient/resource last-success rollups, conflicts, and stale-data warnings.
...
- [ ] Add broader patient/resource last-success rollups, alerts, dead-letter runbook depth, and remaining EDW/local-matching breadth for tenant-specific patient-detail needs.
...
- EHR integration risk: SMART launch can appear complete while still lacking vendor sandbox evidence, remaining FHIR-read/token audit surfaces, durable patient/resource sync status, access-policy attribution, and tenant-specific EDW/local-matching breadth.
- Data risk: Bulk Data kickoff/poll/import/scheduling can create a false sense of production readiness until vendor sandbox evidence, deeper replay/dead-letter runbooks, tombstone behavior, and broader patient/resource last-success rollups are proven.
