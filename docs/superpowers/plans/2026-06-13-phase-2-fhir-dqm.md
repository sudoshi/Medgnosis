...
### Epic A Convergence — ACHIEVED (2026-06-14)

After Epics B+C shipped, the convergence work was completed: **CMS122 now computes a real, non-zero result on live Medgnosis data — IP=17, denominator=17, numerator=0, score=0** (for a 100-patient sample). numerator=0 is *correct*, not a gap: the demo cohort's HbA1c values are all ≤7.6% (zero >9%), so 0% poor control is the true measure result. SQL side stays 0 (`measureCalculatorV2` doesn't compute CMS122v12), so `reconcile()` reports denom delta 17 / agree=false — the honest, quantified CQL-vs-SQL signal.

Five fixes, each found by probing the engine against the Phase-1 MADiE deck (which the engine computes correctly, ip=1 at its native 2026 period):
1. **Period retarget** → migration 058 moves the binding to CY2024 (demo data ends 2025-03; eval is period-relative — even MADiE patients score 0 at 2024).
2. **Encounter export** → `mapEncounterToFHIR` + `encounterCrosswalk` (local `encounter_type` ambulatory/outpatient/urgentcare → Office Visit CPT 99213; wellness → SNOMED check-up) + a period-bounded indexed encounter query.
3. **Condition clinicalStatus** → EDW `diagnosis_status` is null for all cohort diabetes rows; the mapper defaulted null→`resolved`, which broke QI-Core `prevalenceInterval` overlap. Now defaults null→`active`.
4. **QI-Core profiles** (the decisive fix) → the engine's CQL retrieves filter by `meta.profile`: `[Encounter: "Office Visit"]`→`qicore-encounter`, `[ConditionEncounterDiagnosis]`→`qicore-condition-encounter-diagnosis`, HbA1c→`qicore-observation-lab`. The US-Core mappers don't assert these; `qicoreExport` now tags every exported resource with its QI-Core profile (US Core remains asserted too).
5. **Cohort = the engine's value-set expansion** → the local VSAC load and the MADiE bundle share OIDs but have *different* expansions. The reconcile entrypoint now fetches the Diabetes value set from the engine (`GET /ValueSet/{oid}`) and selects/filters the cohort by those codes, so exported conditions are recognizable to the measure's CQL.

**Generalizable lesson:** real eCQM evaluation requires QI-Core-profiled resources AND cohort/value-set codes drawn from the *same* expansion the engine uses — not just structurally-valid FHIR. The encounter-type crosswalk is the EDW→QI-Core data-quality investment per measure.
...
### Epic C Notes (2026-06-14)

- **DEQM 5.0.0 conformance: 0 errors / 0 warnings.** `validator_cli.jar -ig hl7.fhir.us.davinci-deqm#5.0.0 -level error` passes on the golden Gaps-in-Care bundle (`apps/api/test-fixtures/fhir/deqm/gaps-in-care-sample.json`, regenerated by `scripts/deqm-sample.ts`, validated by `scripts/deqm-validate.sh`, wired into the `fhir-conformance` CI job). The validator drove the conformance corrections (13 → 2 → 0 errors): gap status is a **modifierExtension** bound to `…/CodeSystem/gaps-status` (open-gap/closed-gap/prospective-gap); `DetectedIssue.code` is fixed to `v3-ActCode#CAREGAP` (display "Caregap"); the document Bundle needs an `identifier` + absolute `fullUrl`s + an in-bundle `Patient` so `Composition.subject` resolves.
- **WebSocket parity confirmed.** Phase 2 added only new files (`deqm/`, `measureOps`, `qicoreExport`, `cqlEngineLoader`, `measureReportStore`, `qrdaCat1`); it touched no realtime/socket/alert/care-gap-engine code. The live care-gap UX is unchanged — DEQM is a purely additive, payer-exchangeable emission over the same `care_gap` rows.
