...
| 2026-02-26 | Star Schema v2 Consolidation ✓ | **Migration conflict resolution + Phase D verification.** Discovered two parallel migration tracks (Track A: 010_star/011_seed/013_etl vs Track B: 013_enhancement/014_etl) with overlapping table definitions and incompatible column schemas (e.g., dim_payer missing payer_code/is_government, fact_patient_bundle_detail using days_since_last_action vs days_overdue). Archived 3 Track A files, enhanced 013_star_schema_enhancement.sql to be the consolidated canonical definition. Registered 17 previously-applied migrations in `_migrations`. Created `024_star_v2_enhancements.sql` delta migration: ALTER dim_measure (+7 columns), ALTER fact_care_gap (+4 columns + 3 FKs), 3 new fact tables (immunization, insurance, sdoh), 25+ performance indexes, 4 new materialized views (mv_patient_dashboard, mv_bundle_compliance_by_provider, mv_population_overview, mv_care_gap_worklist). Updated `refresh_star_views.sql` to refresh all 7 mat views. Phase D verification passed: 0 FK orphans, sub-ms query performance (0.085ms–0.191ms), all 25 star tables populated, 7 mat views with data. Build: ✓ 4/4 packages clean. |
...
| 2026-02-26 | Tier 2: Wire Real Data ✓ | **Dashboard + Settings wired to live data.** Dashboard API: replaced 3 placeholder queries with real star schema queries — risk stratification from `phm_star.fact_patient_composite` GROUP BY `risk_tier` (was `Promise.resolve([])`), care gap priority breakdown from `phm_edw.care_gap` GROUP BY priority (was all 0s), 30-day encounter count (was 0); high-risk stats card now derived from risk distribution results (count + percentage). Settings: added `PATCH /auth/me` endpoint for profile save (dynamic SET clause, parameterized), `GET/PATCH /auth/me/preferences` endpoints (JSONB column on `app_users`, `||` merge operator), migration `025_user_preferences.sql`. Frontend: wired ProfileSection to call PATCH /auth/me + update auth store on success; NotificationsSection and DataSection toggles now persist via debounced PATCH to `/auth/me/preferences` (400ms debounce, hydrate from server on mount). Added 3 hooks to useApi.ts: `useUpdateProfile`, `useUserPreferences`, `useSavePreferences`. Build: ✓ 4/4 packages clean. |
| 2026-02-26 | Global Icon Scaling ✓ | **All 160+ lucide-react icons now scale with viewport.** Root cause: `size={}` prop sets SVG presentation attributes in fixed `px`, which don't respond to the fluid `html { font-size: clamp(17px, 1.1vw + 1.2px, 21px) }` already in globals.css. Fix: added `svg.lucide { width: 1em; height: 1em; flex-shrink: 0; }` to globals.css — CSS rules override SVG presentation attributes, so every icon now inherits `1em` from its font context, which ultimately traces to the clamp root font-size. Effect: icons scale from ~17px at 1440px to ~21px at 1920px with no changes to individual components. Exception: 10 intentionally-large empty-state icons (size={24}) across AlertsPage, MeasuresPage, 7 patient tab components, and AbbyTab — added `text-2xl` (= 1.5rem) to each `className` so their `1em` resolves to ~25px at 1440px and ~31px at 1920px, preserving visual prominence inside `w-14 h-14` circle containers. |
| 2026-02-26 | Login Page Refinements ✓ | **Layout flip + fluid type + brand relocation.** (1) Flipped split-screen: viz panel moved to left (58%), form to right (42%); divider line moved to left edge of form panel. Hero caption centered vertically and horizontally (`justify-content: center`, `padding: 60px`). (2) Font replaced: Syne → EB Garamond (400–800 from Google Fonts) across all display text. (3) Fluid type scale: added `font-size: clamp(14px, 1.1vw, 22px)` to `.lpg` root; all 19 child font-size declarations converted from fixed `px` to `em` — text now scales from ~14px at 1280px to ~21px at 1920px to 22px cap at 4K. (4) Brand relocation: removed entire logo block (icon + wordmark + subtext) from form panel; "Medgnosis" now renders as a standalone `lpg-brand` title (EB Garamond 600, 2.5em, centered) in the hero panel above the headline. Form panel starts directly with "Welcome back" heading. |
| 2026-02-26 | Login Page Redesign ✓ | **"Patient Grid" — full split-screen login redesign (Clinical Obsidian v2).** Rewrote `LoginPage.tsx` from minimal centered card (Phase 9 blob+grid) to production-quality split-screen: 42% form panel left, 58% animated SVG population network right. Left panel: Syne 700 logo (Fira Code "MG" monogram), controlled inputs (useState), password show/hide toggle (Eye/EyeOff from lucide-react), custom styled checkbox "Keep me signed in", demo quick-fill button (dr.udoshi@medgnosis.app / password), HIPAA · SOC 2 Type II pill badge, live-dot status footer. Right panel: `PopulationNetwork` SVG component — 26 nodes (teal=normal, amber=high-risk, dim=inactive across 5 rows), 45+ edges, 7 ripple rings on high-activity nodes; ambient teal/amber radial washes via ::before/::after pseudos with wash-pulse keyframe; edge vignette for depth; caption overlay with "Every patient. Every gap. One view." headline, stat row (1,288 patients / 26,967 care gaps / 45 bundles in Fira Code monospace). Font stack: Syne (headings/logo), DM Sans (form body — from globals), Fira Code (monogram + stat numbers). All CSS embedded in `<style>` tag — @import Syne from Google Fonts + 12 keyframe animations (card-rise, field-in, node-glow, node-risk, node-dim, edge-pulse, ripple-expand, wash-pulse, btn-shim, err-shake, do-spin) + responsive collapse at 960px. Full feature parity with MindLog login. `live-dot` class from globals.css. TypeScript: ✓ clean (no errors). |
| 2026-02-26 | Tier 3: AI Briefing + Auth + Settings ✓ | **AI Morning Briefing, auth fixes, settings completion.** Created `POST /insights/morning-briefing` endpoint: fetches top 5 high-risk patients from `fact_patient_composite` (by `abigail_priority_score DESC`), today's schedule count, critical alert count; builds clinician-personalized prompt; calls `generateCompletion()` (512 tokens, temp 0.4); returns structured briefing with metadata. Dashboard: `useMorningBriefing` hook auto-fetches on mount (staleTime 30min, retry false), briefing text replaces Abby widget greeting; `abby_briefing.enabled` changed to `true`. **Auth fix:** AppShell logout now calls `POST /auth/logout` to revoke all refresh tokens server-side before clearing local state (was local-only `clearAuth()`). SecuritySection "Sign out all devices" button wired to same endpoint with loading state + toast feedback, redirects to login. 2FA button disabled with "coming soon" message. **Settings DB Overview:** Added `GET /auth/me/db-overview` endpoint (parallel COUNT queries on patient/encounter/procedure/care_gap tables). Extracted `DbOverviewPanel` component using `useDbOverview` hook; replaces hardcoded "~1M records" strings with live counts. Build: ✓ 4/4 packages clean, 2726 modules. |
| 2026-02-26 | Tier 4: Schedule Config + Care Gaps ✓ | **Provider schedule management + care gap workflow enhancements.** **Schedule Config:** Added `GET /auth/me/schedule` endpoint — resolves provider via `app_users.org_id` → `provider.org_id`, fetches weekly schedule from `provider_schedule` (grouped by day_of_week) + clinic resources; `PATCH /auth/me/schedule` accepts array of slot updates (start_time, end_time, slot_duration_min, schedule_type) using `sql.unsafe()` dynamic SET pattern. Rewrote `ScheduleSection` in SettingsPage: 3 panels — (1) Weekly Clinic Hours with editable `<input type="time">` per slot, schedule_type dropdown (clinic/telehealth/admin/off with colored badges), Save button; (2) Automated Tasks (ETL/reports dropdowns persisted via preferences); (3) Clinic Resources read-only list with type badges. Added `useProviderSchedule` + `useSaveProviderSchedule` hooks. **Care Gap Enhancements:** Fixed dashboard `priority` → `gap_priority` column name bug (query was silently failing via `.catch()`). Added `'resolved'` to `careGapUpdateSchema` Zod enum (frontend sent 'resolved' but schema rejected it). Enhanced `GET /care-gaps`: added server-side search (ILIKE on patient name + measure name), priority filter (`gap_priority`), added `priority` + `due_date` to response, priority-first ORDER BY. Enhanced `PATCH /care-gaps/:id`: maps 'resolved'→'closed' for DB storage, persists `notes` to `comments` column via COALESCE. Created migration `026_backfill_care_gap_priority.sql` — deterministic backfill of `gap_priority` (15% high, 35% medium, 50% low) + `due_date` for open gaps. CareListsPage: added Priority column (crimson/amber/emerald badges), Due Date column, priority filter button group (all/high/medium/low), updated table header + skeleton layout. Build: ✓ 4/4 packages clean. |
| 2026-02-26 | Tier 5: Dashboard Trends ✓ | **Live month-over-month trend calculations.** Dashboard API: added 10th parallel query with 6 subqueries comparing 30-day rolling windows — patient registrations (current vs prior 30d), encounters (current vs prior 30d), care gap net change (opened_30d + closed_30d → approximate prior open count). Created `calcTrend()` helper: `((current - prior) / prior) * 100`, handles zero-division. Replaced 3 hardcoded `trend: 0` values (`total_patients`, `care_gaps`, `encounters`) with computed percentages; `risk_score.trend` remains 0 (no historical risk snapshots). Frontend required zero changes — `TrendBadge` component already handles positive (green ↑) and negative (red ↓) trends with percentage display. Build: ✓ 4/4 packages clean. |
| 2026-02-26 | Tier 6: CDS Hooks Order Placement ✓ | **FHIR-based order placement via CDS Hooks for care gap resolution.** **Database:** Migration `020_seed_order_catalog.sql` — ALTERed `order_set` (+bundle_code, +bundle_id) and `order_set_item` (+measure_id, +loinc_description, +cpt_description, +ecqm_reference, +guideline_source, +description); seeded 45 order sets (one per disease bundle) and 354 order set items from `Medgnosis_OrderSet_LOINC_CPT.xlsx` with full LOINC/CPT-4 coding; mapped 15 divergent bundle codes (Excel→DB: DM2→DM, CHF→HF, ASTHMA→ASTH, OBESITY→OB, NAFLD→CLD, STROKE→STR, MIGRAINE→MIG, EPILEPSY→EPI, DM1→T1D, PARK→PD, ANEMIA→ANEM, HLD→LIPID, BIPOLAR→BP, TOBACCO→TOB, WOUNDS→WND); added 3 indexes. **API — Orders:** New `routes/orders/index.ts` with 3 endpoints: `GET /orders/worklist` (population-level bundle-grouped care gaps with available orders, paginated by patient, JOINs care_gap→measure_definition→bundle_measure→condition_bundle→order_set→order_set_item, groups in-memory by patient→bundle→measure→orders), `GET /orders/recommendations/:patientId` (single-patient bundle gaps with order catalog), `POST /orders/place` (validates patient+gap+item, INSERTs into clinical_order, builds FHIR ServiceRequest with LOINC/CPT coding + SNOMED category mapping, updates care_gap status to 'in_progress', audit logs). `buildFhirServiceRequest()` helper generates HL7 FHIR R4 ServiceRequest resources with proper identifier, category (5-type SNOMED map), code (LOINC + CPT dual coding), subject reference. **API — CDS Hooks:** New `routes/cds-hooks/index.ts` implementing HL7 CDS Hooks 2.0 spec: `GET /cds-services` discovery endpoint (no auth, returns medgnosis-care-gaps service with order-sign hook + prefetch templates), `POST /cds-services/medgnosis-care-gaps` handler (extracts patientId from context, filters out LOINC codes already in draftOrders, queries open gaps with orders, builds CDS cards with indicator/summary/detail/suggestions/actions containing FHIR ServiceRequest create actions). Mounted at `/cds-services` root path (outside API_PREFIX, no auth per HL7 spec). **Shared:** Added `placeOrderSchema` Zod validation + `PlaceOrderRequest` type. **Frontend:** Complete rewrite of `CareListsPage.tsx` — from flat gap list to patient-centric bundle worklist. New components: `PatientBundleGroup` (collapsible patient row with avatar, MRN, bundle/gap counts, actionable orders badge), `BundleSection` (bundle header with code badge + open/ordered counts, nested measures), `MeasureRow` (priority dot, measure name, LOINC/CPT code chips, ItemTypeBadge with type-specific icons, Place Order button or Ordered status), `OrderPanel` slide-over (order details surface, priority selector with color-coded buttons, optional instructions textarea, live FHIR ServiceRequest JSON preview in `<pre>` block, Confirm & Place Order submission). Stats strip: Open Gaps + Actionable Orders + Patients + Active Bundles. 3 new hooks: `useOrderWorklist`, `useOrderRecommendations`, `usePlaceOrder`. Build: ✓ 4/4 packages clean. |
| 2026-02-26 | Tier 6.1: Order All (Batch) ✓ | **Batch order placement for complex patients.** Added `placeOrderBatchSchema` to shared (patient_id + priority + orders array of {care_gap_id, order_set_item_id}, min 1, max 50). **API:** `POST /orders/place-batch` endpoint — validates patient, bulk-validates all care_gaps + order_set_items via ANY() queries, places all orders in a single DB transaction (INSERT clinical_order + UPDATE care_gap → in_progress for each), generates FHIR ServiceRequests for every order, audit logs batch with order_ids array. **Frontend:** Added `usePlaceOrderBatch` hook (invalidates worklist, recommendations, care-gaps, dashboard). `PatientBundleGroup` now shows "Order All (N)" violet button when patient has >1 actionable orders (open gaps with available order items). `BatchOrderModal` confirmation dialog: scrollable table of all orders to be placed (name, bundle, type badge, LOINC/CPT code), priority selector (routine/urgent/stat), "Confirm & Place N Orders" button with loading spinner. Success toast shows count + patient name. Build: ✓ 4/4 packages clean. |
| 2026-02-26 | Tier 7: Bundles Page ✓ | **Population-level bundle performance page (master-detail).** **API:** Added `GET /bundles/population` endpoint — queries `phm_star.mv_population_by_condition` joined with `phm_edw.condition_bundle`, returns summary (total_bundles, total_patients, weighted avg_compliance, total_open/closed_gaps), 27 bundle rows with disease_category/patient_count/compliance_pct/critical+high_risk counts, and distinct categories list; optional `?category=` filter. Added `GET /bundles/:bundleCode/patients` endpoint — queries `fact_patient_bundle` joined to `dim_patient`, paginated (page/per_page/risk_tier params), sorted by compliance_pct ASC (worst first), returns patient drilldown with measures_met/total_measures/compliance_pct/risk_tier. Both registered before `/:bundleCode` to avoid route conflict. **Frontend hooks:** `useBundlePopulation(category?)` and `useBundlePatients(bundleCode, params)` in useApi.ts. **BundlesPage.tsx** (~500 lines): 2-column layout matching MeasuresPage pattern — left panel (w-[340px] bg-s0) with CategoryGroup collapsible sections (10 disease categories sorted by patient count desc), BundleCard buttons (code badge + name + patient count + mini compliance bar with color-coded fill), search filter (name/code/category), auto-select first bundle on load; right panel with BundleDetailPanel (ArcGauge SVG semi-circle, 4-stat grid [patients/open gaps/closed/critical], risk distribution horizontal stacked bar [crimson/amber/teal], measures table from useConditionBundle, paginated patient table with PatientAvatar + compliance bar + risk_tier badge + Link to /patients/:id). Skeleton loading for both panels. **Navigation:** Added `/bundles` route in App.tsx, "Bundles" nav item with Layers icon in AppShell.tsx (between Measures and Care Lists). Build: ✓ 4/4 packages clean, 2727 modules. |
| 2026-02-28 | Tier 8: Typography & Palette System ✓ | **Complete typography overhaul + runtime-switchable color palettes.** **Typography:** Replaced Lexend + Fira Code font stack with 4-font professional typography system: Crimson Pro (display), Source Serif 4 (headings), Source Sans 3 (body/UI), IBM Plex Mono (data/code). Updated `index.html` Google Fonts import, `tailwind.config.ts` fontFamily (added `display` + `heading`, renamed `ui` → Source Sans 3, `data` → IBM Plex Mono). Created `styles/themes/tokens-base.css` with font stacks, type scale (rem-based fluid from 17px root), spacing scale, border radii, z-index, easing/duration tokens. Created `styles/themes/tokens-dark.css` — canonical "Clinical Teal" dark palette: 5 surface depth layers (darkest→elevated), primary/accent CSS variables with bg/border/glow variants, text hierarchy (primary/secondary/muted/ghost), border system, panel gradients, sidebar vars, and immutable semantic colors (critical/warning/success/info/violet marked NEVER changed by palette). Imported both token files in `globals.css` before @tailwind directives. Updated `globals.css`: body uses `var(--font-body)`, headings use `var(--font-heading)`, code uses `var(--font-mono)`, `.surface` refactored to use CSS variables (`var(--surface-raised)`, `var(--gradient-panel)`, `var(--border-default)`), added `::before` shimmer pseudo-element, body ambient glow uses `var(--primary-glow)`, focus-visible uses `var(--primary)`. **Palette System:** Created `styles/palettes.ts` — 5 palettes (Clinical Teal default, Arctic sky-blue/orange, Sage emerald/lavender, Sapphire royal-blue/gold, Plum violet/teal). Each palette defines `--primary*`, `--accent*`, `--border-hover/focus` overrides; `applyPalette()` clears all managed vars then applies overrides (default palette = tokens-dark.css values). Refactored `stores/theme.ts` from simple light/dark/system to palette-aware store: `paletteId`, `initFromStorage()` (reads `mg_palette` from localStorage, calls `applyPalette`), `setPalette(id)`. Updated `main.tsx`: calls `useThemeStore.getState().initFromStorage()` before first render to eliminate flash. **Settings — Appearance Tab:** Added new "Appearance" tab (Palette icon) to SettingsPage between Profile and Notifications. `PaletteSection` component: grid of palette cards with dual color swatches (primary + accent circles), selected state with primary ring, toast on switch. |
| 2026-02-28 | Tier 9: Admin Panel ✓ | **Full admin interface with 5-tab layout.** **API (`routes/admin/index.ts`):** Expanded from 3 endpoints to 12+. New: `GET /admin/stats` (parallel counts: patients, providers, open gaps, star schema rows, last ETL status), `POST /admin/users` (create user with role validation, INVITE_PENDING password hash, email uniqueness check), `DELETE /admin/users/:id` (soft-delete via is_active=FALSE), `GET /admin/fhir-endpoints` (active FHIR endpoints), `POST /admin/fhir-endpoints` (create), `PATCH /admin/fhir-endpoints/:id` (update), `DELETE /admin/fhir-endpoints/:id` (soft-delete), `POST /admin/fhir-endpoints/:id/sync` (simulate sync — updates last_sync_at + status=connected), `GET /admin/etl-status` (ETL logs + migrations list + star schema row counts), `POST /admin/refresh-mat-views` (sequential REFRESH MATERIALIZED VIEW CONCURRENTLY for all 7 mat views, returns per-view status, 207 on partial failure). Enhanced `GET /admin/users` (joins provider table for provider name), `PATCH /admin/users/:id` (+first_name, +last_name fields), `GET /admin/audit-log` (+event_type filter, aliased columns: audit_id, event_type, target_type, target_id, description, user_email/first_name/last_name). **Frontend (`AdminPage.tsx`, ~1034 lines):** 5-tab admin interface — Dashboard (6 stat cards + last ETL info), Users (table with role badges + status toggle + inline edit + create user modal + delete confirmation), FHIR Endpoints (endpoint cards with status indicators + create/edit modal + sync button + delete), ETL & Database (ETL log table + migrations accordion + star schema row counts + Refresh Mat Views button with loading), Audit Log (paginated table with event_type filter dropdown). Access gated: redirects non-admin users to /dashboard. **Navigation:** Admin nav item (ShieldCheck icon) in AppShell bottom section, visible only when `user.role === 'admin'`. `/admin` route added to App.tsx. |
| 2026-02-28 | Tier 10: Provider Scoping ✓ | **All dashboard and list queries now scoped to logged-in provider's patient panel.** **Auth:** JWT payload now includes `provider_id` (resolved from `app_users.org_id` → `phm_edw.provider` during login and token refresh). Added `provider_id?: number` to `JwtPayload` interface in `plugins/auth.ts`. Admin users (no provider_id) see full population; provider users see only their PCP panel. **Dashboard (`routes/dashboard/index.ts`):** All 10+ parallel queries now accept `request.user.provider_id` and conditionally add `AND p.pcp_provider_id = ${providerId}` or equivalent star schema `provider_key` filter. `calcTrend()` helper added. Risk stratification scoped via `fact_patient_composite.provider_key`. Today's encounters scoped by `e.provider_id`. Alerts scoped by patient PCP (null patient_id alerts visible to all). Trend calculations scoped. **Patients (`routes/patients/index.ts`):** `GET /patients` — count and page queries run in parallel (Promise.all), both filtered by `pcp_provider_id`. Patient detail `GET /patients/:id` — refactored to fire patient lookup and all 8 sub-resource queries in parallel (was sequential: lookup first, then fan-out). **Care Gaps (`routes/care-gaps/index.ts`):** `GET /care-gaps` — scoped via `JOIN patient` + `pcp_provider_id` filter; added `search` param (ILIKE on patient name + measure name), `priority` filter; data + count queries now parallel; ORDER BY priority-first. `PATCH /care-gaps/:id` — maps 'resolved'→'closed' for DB, persists `notes` to `comments` column. **Morning Briefing (`routes/insights/index.ts`):** High-risk patients, schedule count, and critical alerts all scoped by provider. **Dashboard mock schedule:** Added 18 realistic mock encounters (8am–3pm, 20-min slots) with patient names from real DB, visit types (Office Visit/Follow-up/Preventive/New Patient), clinical reasons, linked to real patient_ids. Past encounters shown with reduced opacity + CheckCircle2 icon. |
| 2026-03-12 | Patient Detail Performance ✓ | **Patient encounters 673ms → 0.6ms (1124x), conditions 599ms → 1.1ms (530x).** Root cause: 4 invalid indexes on `condition_diagnosis`, `encounter`, `observation`, `medication_order`, `patient_allergy` (all patient_id indexes). PG planner was falling back to seq scans on 28M+ row tables. Fixed via `REINDEX INDEX CONCURRENTLY` on all 4 invalid indexes. No code changes required — the correct indexes existed but were marked `indisvalid = false` (likely from interrupted CONCURRENTLY builds during migration). After rebuild, all patient sub-resource queries use proper index scans. |
| 2026-03-12 | Dashboard Performance ✓ | **Dashboard from 5s to 26ms (193x).** Migration `030_dashboard_perf.sql`: (1) composite partial index `idx_encounter_active_datetime_patient` on encounter(encounter_datetime, patient_id) WHERE active_ind='Y' — eliminates 750ms seq scans on 28M rows; (2) `idx_patient_pcp_active` on patient(pcp_provider_id) WHERE active_ind='Y'; (3) `idx_care_gap_patient_status` on care_gap(patient_id, gap_status) WHERE active_ind='Y'; (4) `phm_star.mv_dashboard_stats` materialized view — pre-aggregates per provider: patient/encounter/care gap counts, priority breakdown, risk distribution, 30d trends. One NULL-provider row for admin full-population view. Rewrote dashboard route from 10 parallel PG queries to single mat view lookup + 4 fast clinician queries (today's schedule, alerts, recent encounters). Added to `refresh_star_views.sql`. |
| 2026-03-12 | Solr Query Acceleration ✓ | **Apache Solr 9.7 integration for sub-5ms search across 1M+ patients.** **Infrastructure:** Added Solr 9.7 to `docker-compose.demo.yml` on port 8984 (8983 reserved for Parthenon), 2 cores: `search` (patients + care gaps) and `clinical` (encounters, conditions, observations, medications). Solr config with `managed-schema.xml`, `solrconfig.xml`, medical `synonyms.txt` per core. **Package:** Created `@medgnosis/solr` package with: `SolrClient` (undici HTTP, Basic auth, query/update/delete/commit/ping/coreStatus), search-query builder (patient/care-gap routing with edismax, provider scoping via `fq`), clinical-query builder (4 doc types), 6 cursor-paginated batch indexers (patients, care-gaps, encounters, conditions, observations, medications), full-reindex script with PG advisory lock + ETL logging, CDC listener (PG LISTEN/NOTIFY + Redis overflow queue + batched delta reindex), benchmark runner with percentile stats. **Migration:** `029_solr_cdc_triggers.sql` — NOTIFY triggers on patient/care_gap/encounter/condition_diagnosis/medication_order/observation + `updated_at` columns. **API Integration:** Fastify Solr plugin (`plugins/solr.ts`) with `SOLR_ENABLED` feature flag (default false), graceful degradation to PG on Solr failure, `X-Query-Source` response header. Routes modified: `search/index.ts` (global search → Solr), `patients/index.ts` (patient list search → Solr with PG fallback), `care-gaps/index.ts` (search/filter → Solr with PG fallback). Admin status endpoint enhanced with Solr core stats. **Benchmark Results (20 samples, p50):** Global search: 343ms → 2.4ms (**145x**), Care gaps search: 134ms → 1.7ms (**79x**), Patient list search: 5.4ms → 2.1ms (**2.6x**). Dashboard/conditions/encounters stay on PG (aggregation queries, by design). Search core indexed: 1,005,791 patients + 26,967 care gaps. Build: ✓ clean. |
