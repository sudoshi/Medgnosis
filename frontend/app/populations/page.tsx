  ...
  const filteredPatients = useMemo(() => {
    return mockPatientsList.filter(patient => {
      ...
      if (filters.careGapComplexity) {
        const complexity = patient.careGaps.length > 2 ? 'high' :
                         patient.careGaps.length > 0 ? 'medium' : 'low';
    ...
    const comorbidityDistribution = [
      {
        ...
        percentage: Math.round((filteredPatients.filter(p => p.conditions.length >= 2).length / totalPatients) * 100)
      },
      {
        label: '3+ Conditions',
        count: filteredPatients.filter(p => p.conditions.length >= 3).length,
        percentage: Math.round((filteredPatients.filter(p => p.conditions.length >= 3).length / totalPatients) * 100)
      },
      {
        label: '4+ Conditions',
        count: filteredPatients.filter(p => p.conditions.length >= 4).length,
        percentage: Math.round((filteredPatients.filter(p => p.conditions.length >= 4).length / totalPatients) * 100)
      }
    ];

    const careGapMetrics = {
      total: filteredPatients.reduce((sum, p) => sum + p.careGaps.length, 0),
      overdue: filteredPatients.reduce((sum, p) => sum + p.careGaps.filter(g => g.status === 'open').length, 0),
      trend: -12.3 // Mock trend
    };

    const riskDistribution = {
      high: filteredPatients.filter(p => p.riskFactors.level === 'high').length,
      medium: filteredPatients.filter(p => p.riskFactors.level === 'medium').length,
      low: filteredPatients.filter(p => p.riskFactors.level === 'low').length,
      trendingUp: filteredPatients.filter(p => p.riskFactors.trending === 'up').length
    };
    ...
    <AdminLayout>
      <div className="flex h-[calc(100vh-4rem)]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <h1 className="text-2xl font-semibold">Population Management</h1>
