...
export default function PopulationFilters({ onFiltersChange }: PopulationFiltersProps) {
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set(['conditions']));
    ...
    switch (groupId) {
      case 'conditions':
        if (newFilters.conditions.includes(optionId)) {
          newFilters.conditions = newFilters.conditions.filter(id => id !== optionId);
        } else {
          newFilters.conditions = [...newFilters.conditions, optionId];
        }
        break;
      case 'comorbidity':
        newFilters.comorbidityCount = optionId === '2plus' ? 2 : optionId === '3plus' ? 3 : 4;
        break;
      case 'careGaps':
      ...
      case 'risk':
        if (optionId.includes('risk')) {
          newFilters.riskLevel = optionId.split('_')[0] as 'high' | 'medium' | 'low';
            ...
            {expandedGroups.has(group.id) && (
              <div className="space-y-2 pl-2">
                {group.options.map((option) => {
                  const isActive = activeFilters.conditions.includes(option.id) ||
                    (activeFilters.comorbidityCount === 2 && option.id === '2plus') ||
                    (activeFilters.comorbidityCount === 3 && option.id === '3plus') ||
                    (activeFilters.comorbidityCount === 4 && option.id === '4plus') ||
                    activeFilters.careGapComplexity === option.id ||
                    (activeFilters.riskLevel === option.id.split('_')[0] && option.id.includes('risk')) ||
                    (activeFilters.trending === option.id.split('_')[1] && option.id.includes('trending'));
