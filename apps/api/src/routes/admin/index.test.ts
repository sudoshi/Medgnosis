  ...
  it('returns worker queue and EHR Bulk readiness sections', async () => {
    ...
    expect(res.json()).toMatchObject({
      ...
      data: {
        ...
        ehr_tenants: {
          status: 'blocked',
          tenants: { active: 2, healthy: 1, blocked: 1 },
          backend_services: { ready_for_token_exchange: 1 },
          fhir_api: { failed_requests_24h: 6, auth_failures_24h: 3 },
          resource_coverage: { tenants_missing_required_bulk_coverage: 1 },
        },
        standards: {
          status: 'ok',
          checks: [
